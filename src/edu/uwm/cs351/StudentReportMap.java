...
public class StudentReportMap extends AbstractMap<Student,Grade> {
	...
	private boolean wellFormed() {
		...
		int count = 0;	
		if(firstAtRisk != null) {
			if(firstAtRisk.prevAtRisk != null) return report("the first node previous pointer is not null!");
			RecordNode p = firstAtRisk; 
			while(p != null) {
				if(p.nextAtRisk != null && p.nextAtRisk.prevAtRisk != p) return report("AtRisk Nodes are not correctly doubly linked!");
				if(getNode(p.entry.getKey()) == null) return report("node in AtRisk is not in BST!");
				p = p.nextAtRisk;
				count++;
			}
		}
		if(count != manyAtRisk) return report("AtRisk size is actually " + count + ", while manyAtRisk is " + manyAtRisk + "!");
	...
	private int checkTree(RecordNode r, Student lo, Student hi, RecordNode p) {
		if (r == null)
			return 0;
		if (!checkInAtRisk(r) && (r.nextAtRisk != null || r.prevAtRisk != null)) 
			return reportNeg("node not founded in BST and is in AtRisk!");
		if (r.entry == null)
	...
	private boolean checkInAtRisk(RecordNode r) {
		RecordNode p = firstAtRisk;
		while(p != null) {
			if(p == r) return true;
			p = p.nextAtRisk;
	...
	private RecordNode doAdd(RecordNode n, RecordNode par, Entry<Student, Grade> e) {
		...
		else if (n.entry.getKey() == e.getKey()) {
			n.entry.setValue(e.getValue());
		}
		else {
	...
	public boolean containsKey(Object arg0) {
		...
		assert wellFormed(): "invariant broken at start of contains(Object arg0)"; 
		if(!(arg0 instanceof Student)) return false;
		if(getNode((Student) arg0) != null) return true;
		return false;
	}
	...
	public Grade get(Object arg0) {
		//TODO: Implement get
		assert wellFormed(): "invariant broken at start of get(Object arg0)"; 
		if(!(arg0 instanceof Student)) return null;
		Iterator<Entry<Student,Grade>> it = new MyIterator();
		while(it.hasNext()) {
			Entry<Student, Grade> temp = it.next();
			if(((Student) arg0).getName().equals(temp.getKey().getName())) return getNode((Student) arg0).entry.getValue();
		}
		return null;
	}
	...
	public Grade put(Student s, Grade g) {
		...
		assert wellFormed(): "invariant broken at start of put(Student s, Grade g)"; 
		if(s == null) throw new IllegalArgumentException("can't have null keys in map!");
		if(g == null) throw new IllegalArgumentException("can't have null values in map!");
		RecordNode n = getNode(s);
		if(n != null && n.entry.getValue().equals(g)) return g;
		Grade previous = null;
		if(n == null) {
			manyNodes++;
			...
			previous = n.entry.getValue();
			n.entry.setValue(g);
		}
		version++;
		if(g.getGrade() <= AT_RISK_GRADE) atRiskSet().add(s);
		assert wellFormed(): "invariant broken at end of put(Student s, Grade g)"; 
	...
	public Grade remove(Object arg0) {
		...
		assert wellFormed(): "invariant broken at start of remove(Object o)"; 
		if(!(arg0 instanceof Student)) return null;
		RecordNode n = getNode((Student) arg0);
		if(n == null) return null;
		Grade previous = n.entry.getValue();
		doRemove(getNode((Student) arg0));
		manyNodes--;
		version++;
		assert wellFormed(): "invariant broken at end of remove(Object o)"; 
		return previous;
	}
	...
	private class EntrySet extends AbstractSet<Entry<Student,Grade>> {
		...
		public boolean contains(Object arg0) {
			...
			if(((Entry<?, ?>) arg0).getKey() == null) return false;
			if(((Entry<?, ?>) arg0).getValue() == null) return false;
			if(!((((Entry<?, ?>) arg0).getKey()) instanceof Student)) return false;
			if(!containsKey(((Entry<?, ?>) arg0).getKey())) return false;
			if(!((Entry<?, ?>) arg0).getValue().equals(get(((Entry<?, ?>) arg0).getKey()))) return false;
			return true;
		}
		...
		public boolean remove(Object arg0) {
			...
			doRemove(getNode((Student) ((Entry<?, ?>) arg0).getKey()));
			manyNodes--;
			version++;
			assert wellFormed();
			return true;
	...
	public class AtRiskSet extends AbstractSet<Student> {
		...
		public boolean add(Student e) {
			...
			if(firstAtRisk == null) firstAtRisk = n;
			else {
				RecordNode p = firstAtRisk; 
				firstAtRisk = new RecordNode(n.entry);
				firstAtRisk.nextAtRisk = p;
				p.prevAtRisk = firstAtRisk;
			}
			manyAtRisk++;
			version++;
			assert wellFormed();
		...
		public boolean contains(Object arg0) {
			...
			RecordNode p = firstAtRisk;
			while(p != null) {
				if(p.entry.getKey().equals(n.entry.getKey())) return true;
				p = p.nextAtRisk;
			}
			return false;
		}
		...
		public boolean remove(Object arg0) {
			...
			assert wellFormed();
			if(!(arg0 instanceof Student)) return false;
			if(!contains((Student) arg0)) return false;
			atRiskSet().removeNode(getNode((Student) arg0));
			assert wellFormed();
			return true;
		}
		...
		private boolean removeNode(RecordNode n) {
			...
			while(p != null) {
				if(p.entry == n.entry) {
					if(p.prevAtRisk == null) {
						firstAtRisk = firstAtRisk.nextAtRisk;
						if(firstAtRisk != null) firstAtRisk.prevAtRisk = null;
					}
					else if(p.nextAtRisk == null) {
						p.prevAtRisk.nextAtRisk = null;
						p.prevAtRisk = null;
					}
					else {
						p.prevAtRisk.nextAtRisk = p.nextAtRisk;
						p.nextAtRisk.prevAtRisk = p.prevAtRisk;
						p = null;
					}
					manyAtRisk--;
					version++;
		...
		public Grade getGrade(Student s) {
			...
			RecordNode n = getNode(s);
			if(n == null || !contains(s)) return null;
